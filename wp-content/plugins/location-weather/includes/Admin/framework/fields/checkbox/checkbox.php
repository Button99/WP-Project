...
if ( ! class_exists( 'SPLWT_Field_checkbox' ) ) {
	/**
	 *
	 * Field: checkbox
	 *
	 ...
	 * @version 1.0.0
	 */
	class SPLWT_Field_checkbox extends SPLWT_Fields {
		...
		public function render() {
			...
			$inline_class = ( $args['inline'] ) ? ' class="splwt-lite--inline-list"' : '';
			...
			if ( isset( $this->field['options'] ) ) {
					...
					foreach ( $options as $option_key => $option_value ) {
						...
						if ( is_array( $option_value ) && ! empty( $option_value ) ) {
							...
							foreach ( $option_value as $sub_key => $sub_value ) {
								...
								echo '<span class="splwt-lite--text">' . esc_attr( $sub_value ) . '</span>';
								echo '</label>';
								echo '</li>';
							}
							echo '</ul>';
							...
							echo '<span class="splwt-lite--text">' . esc_attr( $option_value ) . '</span>';
							echo '</label>';
							echo '</li>';
				...
				echo '<label class="splwt-lite-checkbox">';
				echo '<input type="hidden" name="' . esc_attr( $this->field_name() ) . '" value="' . $this->value . '" class="splwt-lite--input"' . $this->field_attributes() . '/>'; // phpcs:ignore
